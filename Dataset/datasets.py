...
def get_dataset(args):
    ...
    #####################################   
        ...
        """
        save_dir = args.root_dir+"/Data/Processed/UTKFace_npy_"+str(constants.IMAGE_SIZE)+"/"
        if constants.DATA_PREPROCESSING == 1:
            print("preprocess and save data.... ")
...
def dP_labels(train_labels):
    if constants.DATASET == "utk_face":
        noise = np.random.laplace(0, constants.K_S / constants.dp_epsilon, constants.TRAIN_SHARE)
        temp = train_labels[:,1] + noise
        train_labels[:,1] = temp
        train_labels[:,1] = np.where(train_labels[:, 1] < 0, 0,
                                          (np.where(train_labels[:, 1] > constants.K_S-1, constants.K_S-1,
                                                    np.around(train_labels[:, 1]))))
        return train_labels
...
def prepare_labels(train_labels, test_labels, valid_labels=None):
    ...
    #####            
        ...
        if  (constants.HONEST==2 and constants.K_Y == 2) or (constants.CURIOUS==2 and constants.K_S == 2):                                         
            ...
            train_labels[:, 2] = np.where(train_labels[:, 2] == 0, 0,
                                (np.where(train_labels[:, 2] == 1, 1, 
                                (np.where(train_labels[:, 2] == 2, 2,
                                (np.where(train_labels[:, 2] == 4, 4, 3)))))))
            test_labels[:,  2] = np.where(test_labels[:,  2] == 0, 0,
                                (np.where(test_labels[:,  2] == 1, 1, 
                                (np.where(test_labels[:,  2] == 2, 2,
                                (np.where(test_labels[:,  2] == 4, 4, 3)))))))                                       
        else:
            if not (constants.HONEST==1 or constants.CURIOUS==1):
