...
if __name__ == "__main__":
    ...
    args.root_dir = ".."
    args.run_name = "test_diff_privacy"
    args.task = "train"
    global constants
    constants.BETA_Y = 5
    constants.BETA_S = 5
    constants.BETA_X = 2
    for dp_epsilon in range(1,11,2):
        constants.dp_epsilon = dp_epsilon*1.0 / 10
        for k_y in range(2, 5):
            constants.K_Y = k_y
            for k_s in range(2, 5):
                constants.K_S = k_s
                sensitivity = constants.K_S
                dp_epsilon = 1
                ## Fetch the datasets
                if constants.DATASET == "utk_face":
                    ...
                    train_labels = datasets.dP_labels(train_labels)
                ...
                exp_name = str(constants.HONEST) + "_" + str(constants.CURIOUS) + "_" + str(constants.K_Y) + \
                           ...
                           "_" + str(int(constants.BETA_Y)) + "_" +str(constants.dp_epsilon) + "/" + str(constants.IMAGE_SIZE) +\
                           "_" + str(constants.RANDOM_SEED)

                log_dir = args.root_dir + "/results/parDP/" + constants.DATASET + "/" + exp_name + "/"
                ...
                if args.task == "fairness":
                    ...
                    data_loader = DataLoader(TensorDataset(torch.Tensor(total_image), torch.Tensor(total_labels).long()),
                                             batch_size=len(test_images) // 50, shuffle=False, drop_last=False)
                    acc_1, acc2, fair_features_count, fair_features_correct = task.evaluate_fairness_par(args, model,
                                                                                                         param_G,
                                                                                                         data_loader)
                    fair_acc_lst = []
                    ...
                    result = task.pass_fairness(EFG, min_fair_label_count, constants.K_Y, constants.K_S, constants.epsilon,
                                                constants.delta)
